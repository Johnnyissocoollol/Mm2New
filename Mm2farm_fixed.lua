...
local function flingMurdererAtRoundEnd()
    ...
    task.spawn(function()
        -- If WE are the murderer, just let auto-reset handle it normally
        if isLocalPlayerMurderer() then return end

        getgenv().OldPos = nil

        -- retry finding murderer for up to 5 seconds
        local murderer = nil
        ...
        -- Check if murderer is our friend
        ...
        if friendCheckResult then
            -- Friend is the murderer: just auto-reset as normal, no fling
        ...
        -- Not a friend: fling them, with up to MAX_FLING_RETRIES retries if we die mid-fling
        isFlingInProgress = true
        flingRetryCount = 0

        while flingRetryCount < MAX_FLING_RETRIES do
            -- Wait for character to be alive and loaded
            local char = localplayer.Character
            ...
            if diedDuringFling then
                -- Died mid-fling: respawn and retry
                flingRetryCount = flingRetryCount + 1
                localplayer.CharacterAdded:Wait()
                task.wait(1.2)
                -- Re-find murderer after respawn
                murderer = findMurderer()
                if not murderer then break end
            else
                -- Fling completed (success or non-death error), stop retrying
                break
            end
        end
...
-- Makes the local character client-invisible for a brief moment (pickup snap frames)
-- Uses LocalTransparencyModifier so only the local client sees it — server unaffected
local function flashClientInvisible(char)
    if not char then return end
    for _, part in ipairs(char:GetDescendants()) do
...
-- Freeze the player in place using PreSimulation (client + physics-level lock).
-- Fires every frame before physics resolves, forcing HRP back to the locked CFrame
-- and zeroing all velocity — the server sees zero movement the entire time.
...
local function doNormalFarm(hrp)
    ...
    -- Re-validate immediately before committing — another player may have grabbed it
    -- in the gap between findNearestCoin and here
    task.wait()
    ...
    -- Watch for coin disappearing mid-tween and cancel early if so
    ...
    -- If coin vanished mid-tween, find next coin that actually has a CoinVisual
    if coinGone then
        local next = findNearestCoin(hrp)
        while next and not isCoinValid(next) do
            visitedCoins[next] = true
            next = findNearestCoin(hrp)
        end
...
local function doSafe2Farm(hrp)
    ...
    if coin and isCoinValid(coin) then
        ...
        if tweenTime > 0 then
            ...
            -- If coin vanished mid-tween, reroute to nearest other coin immediately
            if coinGone then
                anchorHRP(hrp, true)
                local next = findNearestCoin(hrp)
                if next and isCoinValid(next) then
        ...
        -- Flash client invisible for the pickup snap (0.001s) so the standup is hidden
        flashClientInvisible(localplayer.Character)
...
local function applyLayPhysics(hrp)
    -- Clean up any old constraints first
    ...
    -- Remove existing body movers on HRP
    ...
    -- Lock position — no falling / drifting
    local bv = Instance.new("BodyVelocity")
...
local function doLayFarm(hrp)
    ...
    -- Lay mode uses its own offset (3.8), independent of Safe mode
    ...
    if tweenTime > 0 then
        ...
        if coinGone then
            -- Coin gone mid-tween: find next valid coin, skip invalid ones
            anchorHRP(hrp, true)
            local next = findNearestCoin(hrp)
            while next and not isCoinValid(next) do
                visitedCoins[next] = true
    ...
    -- Validate coin still exists after tween
    if not isCoinValid(coin) then
        anchorHRP(hrp, true)
        return
    end
    ...
    -- ── PICKUP SNAP ──────────────────────────────────────────────────────────
    -- Keep PlatformStand forced ON + set gyro to pickup CFrame at the EXACT same
    -- moment as the CFrame snap. This prevents the humanoid animation system from
    -- snapping upright before the gyro corrects it (root cause of the standup flash).
    local char = localplayer.Character
    local humanoid = char and char:FindFirstChild("Humanoid")
    local pickupCFrame = CFrame.new(coin.Position.X, coin.Position.Y, coin.Position.Z) * CFrame.Angles(math.rad(90), 0, 0)

    -- Force PlatformStand on (already on, but re-assert it)
    if humanoid then humanoid.PlatformStand = true end

    -- Sync gyro to pickup position simultaneously with the CFrame snap
    if layGyro and layGyro.Parent then
        layGyro.CFrame = pickupCFrame
    end

    -- Flash client invisible for the snap frame so the brief standup is hidden
    flashClientInvisible(char)
    hrp.CFrame = pickupCFrame
    task.wait(0.001)
    -- ─────────────────────────────────────────────────────────────────────────

    if not isCoinValid(coin) then
        -- Coin gone: snap back underground, stay in lay position
        local retCFrame = CFrame.new(deepPos) * CFrame.Angles(math.rad(90), 0, 0)
        if layGyro and layGyro.Parent then layGyro.CFrame = retCFrame end
        hrp.CFrame = retCFrame
        anchorHRP(hrp, true)
        return
    end

    -- Return underground in lay position
    local retCFrame = CFrame.new(deepPos) * CFrame.Angles(math.rad(90), 0, 0)
    if layGyro and layGyro.Parent then layGyro.CFrame = retCFrame end
    hrp.CFrame = retCFrame
    anchorHRP(hrp, true)
...
-- Shared fling logic: finds murderer, friend-checks, runs SkidFling with retries.
-- flingPos = CFrame to tp to before flinging (nil = stay in place)
...
local function handleRoundEnd(hrp)
    ...
    task.spawn(function()
        if autoResetEnabled then
            -- Auto reset ON: die first, then fling from lobby
            killCharacter()
            -- fallback kill if didn't register
            task.spawn(function()
            ...
            -- wait to be dead and respawned in lobby
            ...
            localplayer.CharacterAdded:Wait()
            task.wait(1)
            -- now fling from lobby position
            runFling(CFrame.new(13.6, 504.8, -50.2))
        else
            -- Auto reset OFF: fling from current position on map, no tp after
            runFling(nil)
            -- do NOT teleport to lobby — stay on map
        end
    end)
end
...
local function customDeathHandler()
    ...
    visitedCoins = {}
    removeInvisibleFloor()
    -- Only reset fling state if we're NOT mid-fling retry (so retry logic can continue)
...
-- ========== SPECTATING / MID-ROUND JOIN DETECTION ==========

-- Returns true if the local player currently has no character (spectating)
-- OR if their character is at the spectator camera position / out of the map.
local function isPlayerSpectating()
    ...
    local humanoid = char:FindFirstChild("Humanoid")
    -- Humanoid health 0 + no death yet = spectating state
    if not humanoid or humanoid.Health <= 0 then return true end
    -- Check if character is way up high at the spectator/lobby area (Y > 400)
    -- while a round is actively running — means they spawned as spectator
    if hrp.Position.Y > 400 and isRoundActive() then return true end
    return false
end

-- Blocks until the player is actually in a playable state (not spectating a mid-round join).
-- If a round is already active when farm is toggled on, we wait for it to finish
-- and for a fresh round to begin before doing anything.
local function waitIfSpectatingMidRound()
    -- No ongoing round = we're fine, nothing to wait for
    if not isRoundActive() then return end

    -- Round is active — check if we're spectating it
    if not isPlayerSpectating() then return end

    -- We joined mid-round as a spectator, hold off
    isSpectatingMidRound = true
    print("[AutoFarm] Joined mid-round as spectator — waiting for round to end before farming.")

    -- Wait for round to end (coins disappear)
    while autoFarm and isRoundActive() do
        task.wait(0.5)
    end

    -- Now wait for the new round's map to load
    if autoFarm then
        waitForNewMapToLoad()
    end
...
local function startFarmLoop()
    ...
    task.spawn(function()
        -- ── MID-ROUND JOIN: if we joined as a spectator, wait it out first ──
        ...
        while autoFarm do
            ...
            if deadUntilNextRound then
                ...
                -- Snap to wait spot and lock immediately on respawn (Safe + Lay mode)
                ...
                -- Wait for the current round to fully end and new map to load
                waitForNewMapToLoad()
                unlockFromLobby()
            ...
            -- Always re-hook death handler each loop iteration
            if humanoid then
            ...
            -- ── PRE-ROUND: lock at wait spot until coins appear (Safe + Lay mode) ──
            ...
            while autoFarm and not isRoundActive() and not deadUntilNextRound do
                task.wait(0.1)
                -- re-lock if character respawned mid-wait (Safe + Lay mode)
            ...
            -- unlock the instant coins appear (Safe + Lay mode)
            if farmMode == "Safe" or farmMode == "Lay" then
                unlockFromLobby()
            end
            ...
            -- re-grab hrp in case character reloaded during wait
            char = localplayer.Character
            if not char then task.wait(0.5) continue end
            ...
            -- ── SAFE MODE: dive underground then tween ───────────────────────
            ...
            -- ── LAY MODE: same as Normal but lying flat ────────────────────────
            ...
            -- ── NORMAL MODE: tween above ground ─────────────────────────────
            else
                normalFarmMain(hrp)
            end
        end
...
local function onCharacterAdded()
    ...
    if char then
        ...
        -- Snap to wait spot if autoFarm is on and no coins are up yet (Safe + Lay mode)
...
print("Script loaded - Fling Murder toggle added (activates at round end)")