...
-- ========== NEW TOGGLE STATE ==========
local shootMurderEnabled = false   -- Toggle 1: TP+shoot murderer at round end / retry loop
local autoGrabGunEnabled = false   -- Toggle 2: Auto pick up dropped GunDrop mid-round
local grabGunConnection = nil
local MURDERER_SAFE_DIST = 20
...
-- ========== SHOOT MURDER HELPERS ==========

local function localPlayerHasGun()
    return (localplayer.Backpack and localplayer.Backpack:FindFirstChild("Gun")) or
...
-- Fires the equipped gun tool (simulates activation)
local function fireGun()
    local char = localplayer.Character
    if not char then return end
    local gun = char:FindFirstChild("Gun")
    if not gun then return end
    -- Activate the tool (fires server-side shoot event)
    local activateEvent = gun:FindFirstChild("RemoteEvent") or gun:FindFirstChild("Shoot") or gun:FindFirstChildOfClass("RemoteEvent")
    if activateEvent then
        pcall(function() activateEvent:FireServer() end)
    end
    -- Also try activating via tool
    pcall(function() gun:Activate() end)
end

-- TP next to murderer then fire gun, retry until murderer dies or we lose the gun
local function shootMurderLoop()
    if not shootMurderEnabled then return end

    task.spawn(function()
        -- Find murderer with a deadline
        local murderer = nil
        ...
        local retryDeadline = tick() + 60 -- max 60s of retrying

        while shootMurderEnabled and tick() < retryDeadline do
            -- Do we still have the gun?
            if not localPlayerHasGun() then
                -- Go back to lobby and wait 4.7s, then retry
                local char = localplayer.Character
                if char then
                ...
                -- Recheck gun
                if not localPlayerHasGun() then
                    -- Still no gun, keep retrying with 4.7s waits
                    task.wait(4.7)
                    continue
                end
            end

            -- Re-validate murderer
            ...
            if not murderHRP then
                task.wait(0.3)
                continue
            end

            local murderHum = murderChar:FindFirstChildOfClass("Humanoid")
            if not murderHum or murderHum.Health <= 0 then
                -- Murderer is dead, mission complete
                break
            end

            -- Equip gun if not already in hand
            if not (localplayer.Character and localplayer.Character:FindFirstChild("Gun")) then
                equipGun()
                task.wait(0.2)
            end

            -- TP close to murderer
            local char = localplayer.Character
            if char then
                local hrp = char:FindFirstChild("HumanoidRootPart")
                if hrp then
                    local offset = Vector3.new(3, 0, 0)
                    hrp.CFrame = CFrame.new(murderHRP.Position + offset)
                    task.wait(0.15)
                end
            end

            -- Face murderer and fire
            local myChar = localplayer.Character
            if myChar then
                local myHRP = myChar:FindFirstChild("HumanoidRootPart")
                if myHRP then
                    myHRP.CFrame = CFrame.lookAt(myHRP.Position, murderHRP.Position)
                end
            end
            ...
            murderHum = murderChar:FindFirstChildOfClass("Humanoid")
            if not murderHum or murderHum.Health <= 0 then
                break
            end
        end
...
-- Called at round end: if we have a gun, skip normal flow and do shoot-murder instead
local function tryShootMurderAtRoundEnd()
    if not shootMurderEnabled then return false end
    if isLocalPlayerMurderer() then return false end
    if not localPlayerHasGun() then return false end
...
local function findNearestCoin(hrp)
    ...
    if coinContainer then
        for _, coin in ipairs(coinContainer:GetChildren()) do
            if coin:IsA("BasePart") and coin.Name == "Coin_Server" and isCoinValid(coin) and not visitedCoins[coin] then
...
-- ========== ROUND END (modified to hook shoot-murder) ==========
...
local function handleRoundEnd(hrp)
    ...
    task.spawn(function()
        -- SHOOT MURDER: takes priority over auto-reset if we have the gun
        -- Still respects auto-reset for dying purposes; just skips the wait-in-lobby phase
        if shootMurderEnabled and not isLocalPlayerMurderer() and localPlayerHasGun() then
            -- If auto-reset is on, die first then shoot from lobby
            if autoResetEnabled then
                killCharacter()
                task.spawn(function()
                    local dl = tick() + 3
                    while tick() < dl do
                        ...
                        if not c or not hm or hm.Health <= 0 then return end
                    end
                    local c = localplayer.Character
                    local hm = c and c:FindFirstChild("Humanoid")
                    if hm and hm.Health > 0 then hm.Health = 0 end
                end)
                if localplayer.Character then
                    local hm = localplayer.Character:FindFirstChild("Humanoid")
                    if hm and hm.Health > 0 then hm.Died:Wait() end
                end
                localplayer.CharacterAdded:Wait()
                task.wait(1)
                -- gun should be in backpack after respawn; equip and shoot
                equipGun()
                task.wait(0.2)
            end
            shootMurderLoop()
...
local function isMurdererNearGun(gunDrop)
    ...
    return (murderHRP.Position - gunDrop.Position).Magnitude < MURDERER_SAFE_DIST
end

-- Teleports to the gun, grabs it, then returns to the underground coin position
-- so farming resumes exactly like after the initial lobby-to-first-coin teleport.
local function grabDroppedGunAndResume(gunDrop)
    ...
    -- Wait until murderer is a safe distance away
    while isMurdererNearGun(gunDrop) and autoGrabGunEnabled do
        task.wait(0.5)
    end
    if not autoGrabGunEnabled then return end
    ...
    -- Pause farming briefly
    cancelActiveTween()
    disableNoclip()
    removeInvisibleFloor()
    anchorHRP(hrp, false)

    -- TP to gun to grab it
    hrp.CFrame = gunDrop:GetPivot()
    task.wait(0.35)

    -- Now resume: find the nearest coin and teleport to its underground position,
    -- just like the start of the farming loop does before entering safe2FarmMain/layFarmMain.
    character = localplayer.Character
    if not character then return end
    ...
    if coin and isCoinValid(coin) then
        ...
        forceServerSync(character)
        task.wait(0.1)
    end
    -- The main farm loop will naturally pick back up from here on its next iteration
end

local function startAutoGrabGun()
    if grabGunConnection then return end

    -- Watch for new GunDrops
    grabGunConnection = workspace.DescendantAdded:Connect(function(child)
        if child.Name == "GunDrop" and autoGrabGunEnabled then
            task.wait(0.5)
            task.spawn(grabDroppedGunAndResume, child)
        end
    end)

    -- Also scan existing ones
    task.spawn(function()
        task.wait(1)
...
local function stopAutoGrabGun()
    if grabGunConnection then
        grabGunConnection:Disconnect()
        grabGunConnection = nil
    end
end
...
-- Extended height to fit 2 new rows (+68)
frame.Size = UDim2.new(0, 220, 0, 408)
...
-- Original rows (unchanged Y positions)
...
-- Two new rows
local shootBtn,     shootBg,     shootKnob     = createToggleRow(frame, "Shoot Murder",      244)
local grabGunBtn,   grabGunBg,   grabGunKnob   = createToggleRow(frame, "Auto Grab Gun",     284)
...
setToggleState(shootBg,     shootKnob,     false)
setToggleState(grabGunBg,   grabGunKnob,   false)

-- Mode dropdown — shifted down by 68 to make room
local modeRow = Instance.new("Frame")
modeRow.Size = UDim2.new(1, -28, 0, 34)
modeRow.Position = UDim2.new(0, 14, 0, 312)
...
dropPanel.Position = UDim2.new(1, -110, 0, 304)
...
-- Speed slider — shifted down by 68
local speedRow = Instance.new("Frame")
speedRow.Size = UDim2.new(1, -28, 0, 40)
speedRow.Position = UDim2.new(0, 14, 0, 353)
...
-- Toggle 1: Shoot Murder
shootBtn.MouseButton1Click:Connect(function()
    shootMurderEnabled = not shootMurderEnabled
    setToggleState(shootBg, shootKnob, shootMurderEnabled)
end)

-- Toggle 2: Auto Grab Gun
grabGunBtn.MouseButton1Click:Connect(function()
    autoGrabGunEnabled = not autoGrabGunEnabled
    setToggleState(grabGunBg, grabGunKnob, autoGrabGunEnabled)
